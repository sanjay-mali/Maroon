...
export const getTotalProducts = async (): Promise<number> => {
    try {
        const { total } = await databases.listDocuments(databaseId, productsCollectionId);
        return total;
    } catch (error) {
...
export const getTotalRevenue = async () => {
    try {
        const { documents } = await databases.listDocuments(databaseId, ordersCollectionId);
...
export const getTotalOrders = async () => {
    try {        
        const { total } = await databases.listDocuments(databaseId, ordersCollectionId);
        return total;
    } catch (error) {
...
export const getLowStockProducts = async (): Promise<any[]> => {
    try {
        const { documents } = await databases.listDocuments(databaseId, productsCollectionId, [
...
export const getProductById = async (productId: string): Promise<any | null> => {
    try {
        const product = await databases.getDocument(databaseId, productsCollectionId, productId) as any;
        return {
            id: product.$id,
            images: product.images,
            ...product,
...
export const addProduct = async (productData: any) => {
    try {
        await databases.createDocument(databaseId, productsCollectionId, ID.unique(), { ...productData });
    } catch (error) {
...
export const updateProduct = async (productId: string, productData: any) => {
    try {        
        await databases.updateDocument(databaseId, productsCollectionId, productId, productData);
...
export const getAllCategories = async () => {
    try {        
        const { documents } = await databases.listDocuments(databaseId, categoriesCollectionId);
...
export const getCategoryById = async (categoryId: string) => {
    try {        
        const category = await databases.getDocument(databaseId, categoriesCollectionId, categoryId) as any;
        return {
            id: category.$id,
            name: category.name,
        };
...
export const addCategory = async (name: string) => {
    try {
        await databases.createDocument(databaseId, categoriesCollectionId, ID.unique(), { name });
        return true;
    } catch (error) {
...
export const updateCategory = async (categoryId: string, name: string) => {
    try {
        await databases.updateDocument(databaseId, categoriesCollectionId, categoryId, { name });
        return true;
    } catch (error) {
...
export const deleteCategory = async (categoryId: string) => {
    try {
        await databases.deleteDocument(databaseId, categoriesCollectionId, categoryId);
        return true;
    } catch (error) {
...
export const uploadImages = async (files: File[]) => {
    try {
        ...
        for (const file of files) {            
             ...
             try {
               const storageResponse = await storage.createFile(storageId, ID.unique(), file);
               const previewUrl = storage.getFilePreview(storageId, storageResponse.$id).href;
...
export const getOrderById = async (orderId: string) => {
    try {        
        const order = await databases.getDocument(databaseId, ordersCollectionId, orderId) as any;
        return {
            id: order.$id,
            status: order.status,
            ...order,
...
export const addProductToWishlist = async (userId: string, productId: string) => {
    try {
        ...
        if (!userWishlist.includes(productId)) {
            ...
            await databases.updateDocument(databaseId, usersCollectionId, userId, { wishlist: userWishlist });
        }
        return true;
    } catch (error) {
...
export const getWishlist = async (userId: string) => {
    try {
        const user = await databases.getDocument(databaseId, usersCollectionId, userId);
        const userWishlist = user.wishlist || [];
        const products = await Promise.all(userWishlist.map(async (productId: string) => {
...
export const getAllUsers = async () => {
    try {
        const { documents } = await databases.listDocuments(databaseId, usersCollectionId);
...
export const getUserById = async (userId: string) => {
    try {        
        const user = await databases.getDocument(databaseId, usersCollectionId, userId) as any;
        return {
            id: user.$id,
            email: user.email,
            ...user,
...
export const getAllAddresses = async () => {
    try {
        const { documents } = await databases.listDocuments(databaseId, usersCollectionId);
        return documents.map((user: any) => (user.addresses || []).map((address: any) => ({
...
export const getAddressById = async (addressId: string): Promise<any | null> => {
    try {
        const addresses = await databases.listDocuments(databaseId, usersCollectionId, [
            Query.search('addresses', addressId)
        ]);
        if (addresses.documents.length > 0) {
            const address = addresses.documents[0].addresses.find(address => address.$id === addressId)
            return address;
        }
        return null
    } catch (error) {
